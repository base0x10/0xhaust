...
static const str_toks_t str_toks[] = {
    /* Opcodes */
    {"DAT", TOK_DAT}, {"SPL", TOK_SPL}, {"MOV", TOK_MOV},
    {"DJN", TOK_DJN}, {"ADD", TOK_ADD}, {"JMZ", TOK_JMZ},
    {"SUB", TOK_SUB}, {"MOD", TOK_MOD}, {"CMP", TOK_SEQ},
    {"SEQ", TOK_SEQ}, {"JMP", TOK_JMP}, {"JMN", TOK_JMN},
    {"SNE", TOK_SNE}, {"MUL", TOK_MUL}, {"DIV", TOK_DIV},
    {"SLT", TOK_SLT}, {"NOP", TOK_NOP},

    {"ORG", TOK_ORG}, /* pseudo-ops */
    {"END", TOK_END}, {"PIN", TOK_PIN}, {"START", TOK_START},

    {"F", TOK_mF}, /* modifiers */
    {"A", TOK_mA},    {"B", TOK_mB},    {"AB", TOK_mAB},
    {"BA", TOK_mBA},  {"X", TOK_mX},    {"I", TOK_mI},
    {NULL, 0} /* sentinel */
};
...
static const char *skip_white(char const *s) {
  while (isspace(*s)) s++;
  return s;
}

static const char *get_tok(const char *s, int *tok, char *tok_buf,
                           int *tok_int) {
 ...
 *
 * SYNOPSIS
 *     void panic_bad_token( int tok, const char *expected, char *tok_buf, int
 **tok_int);
 *
 * INPUTS
 ...
 */
static void panic_bad_token(int tok, const char *expected, char *tok_buf,
                            int *tok_int) {
  char *errstr = NULL;
  char buf[30];
 ...
 *
 * SYNOPSIS
 *     int asm_line( const char *line, insn_t *in, unsigned int CORESIZE, char
 * *tok_buf, int *tok_int);
 *
 * INPUTS
...
int asm_line(const char *line, insn_t *in, unsigned int CORESIZE, char *tok_buf,
             int *tok_int) {
...
#define panic_bad_tok(s) (panic_bad_token(tok, s, tok_buf, tok_int))
  ...
  if (!(is_tok_opcode(tok))) panic_bad_tok("an opcode");
  ...
  if (tok != '.') panic_bad_tok("'.'");

  s = get_tok(s, &tok, tok_buf, tok_int); /* then the modifier itself */
  if (!is_tok_modifier(tok)) panic_bad_tok("a modifier");
  m = tok - TOK_mF;

  /* Match a-field addressing mode and a-field
   */
  s = get_tok(s, &tok, tok_buf, tok_int);
  if (!is_tok_mode(tok)) panic_bad_tok("an addressing mode specifier");
  ma = tok - TOK_DIRECT;

  s = get_tok(s, &tok, tok_buf, tok_int);
  if (tok != TOK_INT) panic_bad_tok("an integer");
...
void asm_file(FILE *F, warrior_t *w, unsigned int CORESIZE) {
  ...
  int tok_int = 0; /* if the token was a TOK_INT, the value of the token is
                      returned here */
  ...
  while (fgets(line, MAX_ALL_CHARS, F)) {
    ...
    switch (ret) {
      case ASMLINE_OK:
        if (get_flags(c->in) & fl_START) w->start = w->len;
      ...
      default:
        fprintf(stderr,
                "asm.c/asm_file(): illegal return code from asm_line()\n");
        exit(1);
    }
    if (w->len > MAXLENGTH) {
